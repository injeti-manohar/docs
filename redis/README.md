...
-	[`4.0.12-alpine`, `4.0-alpine`, `4-alpine`, `4.0.12-alpine3.9`, `4.0-alpine3.9`, `4-alpine3.9` (*4.0/alpine/Dockerfile*)](https://github.com/docker-library/redis/blob/60db7082c81f5d457046f199244c5d651f04b3fa/4.0/alpine/Dockerfile)

[![Build Status](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/redis/badge/icon) (`arm32v6/redis` build job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/redis/)
...
```console
$ docker run --name some-redis -d arm32v6/redis redis-server --appendonly yes
```
...
```console
$ docker run -it --link some-redis:redis --rm arm32v6/redis redis-cli -h redis -p 6379
```
...
```console
$ docker run -v /myredis/conf/redis.conf:/usr/local/etc/redis/redis.conf --name myredis arm32v6/redis redis-server /usr/local/etc/redis/redis.conf
```
