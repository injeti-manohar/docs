...
```console
$ docker run -it --link some-mongo:mongo --rm s390x/mongo mongo --host mongo test
```
...
```console
$ docker run --name some-mongo -d s390x/mongo --smallfiles
```
...
```console
$ docker run --name some-mongo -d s390x/mongo --wiredTigerCacheSizeGB 1.5
```
...
For a more complicated configuration setup, you can still use the MongoDB configuration file. `mongod` does not read a configuration file by default, so the `--config` option with the path to the configuration file needs to be specified. Create a custom configuration file and put it in the container by either creating a custom Dockerfile `FROM s390x/mongo` or mounting it from the host machine to the container. See the MongoDB manual for a full list of [configuration file](https://docs.mongodb.com/manual/reference/configuration-options/) options.
...
```console
$ docker run -d --name some-mongo -e MONGO_INITDB_ROOT_USERNAME=mongoadmin -e MONGO_INITDB_ROOT_PASSWORD=secret s390x/mongo

$ docker run -it --rm --link some-mongo:mongo s390x/mongo mongo --host mongo -u mongoadmin -p secret --authenticationDatabase admin some-db
> db.getName();
some-db
```
