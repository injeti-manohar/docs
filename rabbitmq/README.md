...
[![Build Status](https://doi-janky.infosiftr.net/job/multiarch/job/amd64/job/rabbitmq/badge/icon) (`amd64/rabbitmq` build job)](https://doi-janky.infosiftr.net/job/multiarch/job/amd64/job/rabbitmq/)
...
```console
$ docker run -d --hostname my-rabbit --name some-rabbit -e RABBITMQ_ERLANG_COOKIE='secret cookie here' amd64/rabbitmq:3
```

This can then be used from a separate instance to connect:

```console
$ docker run -it --rm --link some-rabbit:my-rabbit -e RABBITMQ_ERLANG_COOKIE='secret cookie here' amd64/rabbitmq:3 bash
...
```console
$ docker run -it --rm --link some-rabbit:my-rabbit -e RABBITMQ_ERLANG_COOKIE='secret cookie here' -e RABBITMQ_NODENAME=rabbit@my-rabbit amd64/rabbitmq:3 bash
