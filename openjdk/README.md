...
[![Build Status](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/openjdk/badge/icon) (`ppc64le/openjdk` build job)](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/openjdk/)
...
```console
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp ppc64le/openjdk:7 javac Main.java
```
...
The `ppc64le/openjdk` images come in many flavors, each designed for a specific use case.

## `ppc64le/openjdk:<version>`
...
## `ppc64le/openjdk:<version>-slim`

This image installs the `-headless` package of OpenJDK and so is missing many of the UI-related Java libraries and some common packages contained in the default tag. It only contains the minimal packages needed to run Java. Unless you are working in an environment where *only* the `ppc64le/openjdk` image will be deployed and you have space constraints, we highly recommend using the default image of this repository.

## `ppc64le/openjdk:<version>-alpine`
