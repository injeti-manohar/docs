...
-	[`11.1`, `11`, `latest` (*11/Dockerfile*)](https://github.com/docker-library/postgres/blob/42f9ab3bab65fdbabbf35130c68a9869b6e82ee7/11/Dockerfile)
-	[`11.1-alpine`, `11-alpine`, `alpine` (*11/alpine/Dockerfile*)](https://github.com/docker-library/postgres/blob/cfac232e3cccb8f3b499b7a286ccdf6eafbde808/11/alpine/Dockerfile)
-	[`10.6`, `10` (*10/Dockerfile*)](https://github.com/docker-library/postgres/blob/45b855af13f6a753fa77bb830c482af6a69d50da/10/Dockerfile)
-	[`10.6-alpine`, `10-alpine` (*10/alpine/Dockerfile*)](https://github.com/docker-library/postgres/blob/cfac232e3cccb8f3b499b7a286ccdf6eafbde808/10/alpine/Dockerfile)
-	[`9.6.11`, `9.6`, `9` (*9.6/Dockerfile*)](https://github.com/docker-library/postgres/blob/45b855af13f6a753fa77bb830c482af6a69d50da/9.6/Dockerfile)
-	[`9.6.11-alpine`, `9.6-alpine`, `9-alpine` (*9.6/alpine/Dockerfile*)](https://github.com/docker-library/postgres/blob/cfac232e3cccb8f3b499b7a286ccdf6eafbde808/9.6/alpine/Dockerfile)
-	[`9.5.15`, `9.5` (*9.5/Dockerfile*)](https://github.com/docker-library/postgres/blob/45b855af13f6a753fa77bb830c482af6a69d50da/9.5/Dockerfile)
-	[`9.5.15-alpine`, `9.5-alpine` (*9.5/alpine/Dockerfile*)](https://github.com/docker-library/postgres/blob/cfac232e3cccb8f3b499b7a286ccdf6eafbde808/9.5/alpine/Dockerfile)
-	[`9.4.20`, `9.4` (*9.4/Dockerfile*)](https://github.com/docker-library/postgres/blob/45b855af13f6a753fa77bb830c482af6a69d50da/9.4/Dockerfile)
-	[`9.4.20-alpine`, `9.4-alpine` (*9.4/alpine/Dockerfile*)](https://github.com/docker-library/postgres/blob/cfac232e3cccb8f3b499b7a286ccdf6eafbde808/9.4/alpine/Dockerfile)

[![Build Status](https://doi-janky.infosiftr.net/job/multiarch/job/amd64/job/postgres/badge/icon) (`amd64/postgres` build job)](https://doi-janky.infosiftr.net/job/multiarch/job/amd64/job/postgres/)
...
```console
$ docker run -it --rm --link some-postgres:postgres amd64/postgres psql -h postgres -U postgres
psql (9.5.0)
Type "help" for help.
...
```console
$ docker run -it --rm --user www-data amd64/postgres
The files belonging to this database system will be owned by user "www-data".
...

$ docker run -it --rm --user 1000:1000 amd64/postgres
...
2.	bind-mount `/etc/passwd` read-only from the host (if the UID you desire is a valid user on your host):
	...
	```console
	$ docker run -it --rm --user "$(id -u):$(id -g)" -v /etc/passwd:/etc/passwd:ro amd64/postgres
...
3.	initialize the target directory separately from the final runtime (with a `chown` in between):
	...
	$ docker run -it --rm -v pgdata:/var/lib/postgresql/data amd64/postgres
...
-	Use a custom config file. Create a config file and get it into the container. If you need a starting place for your config file you can use the sample provided by PostgreSQL which is available in the container at `/usr/share/postgresql/postgresql.conf.sample` (`/usr/local/share/postgresql/postgresql.conf.sample` in Alpine variants).
	...
	$ docker run -d --name some-postgres -v "$PWD/my-postgres.conf":/etc/postgresql/postgresql.conf amd64/postgres -c 'config_file=/etc/postgresql/postgresql.conf'
	```
...
-	Set options directly on the run line. The entrypoint script is made so that any options passed to the docker command will be passed along to the `postgres` server daemon. From the [docs](https://www.postgresql.org/docs/current/static/app-postgres.html) we see that any option available in a `.conf` file can be set via `-c`.
	...
	```console
	$ docker run -d --name some-postgres amd64/postgres -c 'shared_buffers=256MB' -c 'max_connections=200'
	```
...
2.	Start your `amd64/postgres` container like this:
	...
	```console
	$ docker run --name some-postgres -v /my/own/datadir:/var/lib/postgresql/data -d amd64/postgres:tag
	```
